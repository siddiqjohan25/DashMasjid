  ...
  useEffect(() => {
    ...
    axios.get("https://waktu-solat-api.herokuapp.com/api/v1/prayer_times.json")
      .then(function (response) {
        let newArray = response.data.data.negeri.filter(function (el) {
          return el.nama == state
        }
        );
        newArray = newArray[0].zon
        newArray = newArray.filter(function (el) {
          return el.nama == region
        }
        );
        newArray = newArray[0]
        console.log(newArray)
        setimsak(newArray.waktu_solat[0].time)
        setsubuh(newArray.waktu_solat[1].time)
        setsyuruk(newArray.waktu_solat[2].time)
        setzohor(newArray.waktu_solat[3].time)
        setasar(newArray.waktu_solat[4].time)
        setmaghrib(newArray.waktu_solat[5].time)
        setisyak(newArray.waktu_solat[6].time)
        setLoading(false)
      })
  ...
  >
       ...
       <h1 className='flex justify-between my-10'><span className='text-5xl font-bold'>Isyak</span> <span className='text-5xl font-bold'>{isyak}</span></h1>
        ...
        <span className='text-2xl  text-left font-semibold my-2 block'>
         ...
         <div className='text-center text-xl px-40 mt-5'>
